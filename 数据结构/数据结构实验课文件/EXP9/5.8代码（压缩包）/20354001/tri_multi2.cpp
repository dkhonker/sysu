...
void tri_multi() {
	...
	while(row <= Q[0][0]){
		for (int i =  ;  ; i++) //Please fill these blank parts
			for (int j =  ;  ; j++)	{ //Please fill these blank parts
	    ...
	    for(col = 1; col <= Q[0][1]; col++)
	    {
	  		...
	  		if (      ) //Please fill this blank part, IF non zero, than save to the three tuple table
	  		{
        //please add something here
	  		temp[col] = 0;
	  		}
...
int main() {
	...
	{
		if (a[k][0] == sign){
			a_rpos[sign++] = k;   //  a_rpos[i] 代表 第i行 第一个元素的位置 
			k++;
		}
		else if(a[k][0] < sign){
			k++;
