...
void tri_multi() {
	...
	while(row <= Q[0][0]){
		for (int i = a_rpos[row] ;i<a_rpos[row+1]; i++) 
			for (int j = b_rpos[a[i][j]] ; j<b_rpos[a[i][1]+1] ; j++)	{ //Please fill these blank parts
	    ...
	    for(col = 1; col <= Q[0][1]; col++)
	    {
	  		...
	  		if (   temp[col]   ) //Please fill this blank part, IF non zero, than save to the three tuple table
	  		{
	  		if(++Q[0][2]>Maxn)
	  		return;
	  		int k=0;
