...
void tarjan_scc(int v){  // SCC主过程! 
	index[v]=low[v]=count++;
	stack[top++]=v;on_stack[v]=1;// push v into the stack and mark v to be on stack.
	for(int i=p[v]+1;i<=p[v+1];i++){
		...
		else if(on_stack[w]==1) low[v]=min(low[v],index[w]);//Hint 1. dfs. Hint 2. modify low[v]
	}
...
int partition(int array[], int low, int high) {
  ...
  for (int j = low; j < high; j++) {
    if(array[j]>=pivot) {swap(array[j],array[i]);i++;}//fill this part by yourself.
  }
