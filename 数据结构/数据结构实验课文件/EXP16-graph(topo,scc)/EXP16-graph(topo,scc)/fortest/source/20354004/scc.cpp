...
void tarjan_scc(int v){  // SCC主过程! 
	index[v]=low[v]=count++;
	stack[i++]=v;
	on_stack[v]=1;// push v into the stack and mark v to be on stack.
	for(int i=p[v]+1;i<=p[v+1];i++){
		...
		else if(on_stack[w])low[v]=min(low[v],index[w]);//Hint 1. dfs. Hint 2. modify low[v]
	}
	if(index[v]==low[v]){ // find out a new component. 
		int j;
		do{
			j=stack[--i];//Add some code in the following.
