...
void tarjan_scc(int v){  // SCC主过程! 
	index[v]=low[v]=count++;
	on_stack[v]=1;
	stack[++top]=v;// push v into the stack and mark v to be on stack.
	for(int i=p[v]+1;i<=p[v+1];i++){
		...
		if(index[w]==0){
			...
			low[v]=min(low[v],index[w]);//add some code here by yourself
		//Hint 1. dfs. Hint 2. modify low[v]
	}
	if(index[v]==low[v]){ // find out a new component. 
		int j;
		do{
			j = stack[top--];//Add some code in the following.
			on_stack[j]=0;//Hint: pop an element from the stack to j, and mark j to be not on stack.
			size[tComponent]++;
		} while (j!=v);
		tComponent++;
	}
...
int partition(int array[], int low, int high) {
  ...
  for (int j = low; j < high; j++) {
	if(array[j]<=pivot)
		swap(array[i++],array[j]);//fill this part by yourself.
  }
...
void quickSort(int array[], int low, int high) {
  if(low<high){
  	...
  	if(k!=low)quickSort(array,low,k-1);
  	if(k!=high)quickSort(array,k+1,high);
