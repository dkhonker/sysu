...
void tarjan_scc(int v){  // SCC主过程! 
	...
	stack[top]=v;
	on_stack[v]=1;
	top++;// push v into the stack and mark v to be on stack.
	...
	if(index[v]==low[v]){ // find out a new component. 
		...
		do{
			j=stack[--top];		
			on_stack[j]=0;//Add some code in the following.
...
void quickSort(int array[], int low, int high) {
  if(low<high){
	...
	quickSort(array,k+1,high);// write this by yourself.
}
}
