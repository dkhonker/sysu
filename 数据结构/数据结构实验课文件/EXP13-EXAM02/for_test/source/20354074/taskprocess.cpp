...
int taskProcess(){
	...
	while (tasknum) {
		...
		/*Explain: The following loop finds the task j with the earliest
		 possible finishing time among those who can be processed now.*/
		for (int i = 1; i <= n; i++) {
		...
		if (p[j] == 0){	
			cnt+=t;//add something here.  
			tasknum--;/*Hint: Update cnt and tasknum below*/ 
		}
	}
	return cnt;
}
