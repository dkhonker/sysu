...
cout<<root.data;
if (root.lchild<=n&&root.lchild>0)Preorder(nodes[root.lchild]);
if (root.rchild<=n&&root.rchild>0)Preorder(nodes[root.rchild]);
}
void Midorder(Node &root){ //please finish the following parts. It is easier.
if (root.lchild<=n&&root.lchild>0)Midorder(nodes[root.lchild]);
cout<<root.data;
if (root.rchild<=n&&root.rchild>0)Midorder(nodes[root.rchild]);
}
