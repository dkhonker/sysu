	...
	while (f < r){
		...
		for (int i=1; i<=d[u]; i++){
			...
			if(!visited[v]){
				...
				if(A[v][j]==u) B[v][j]=1;
				counter++;
				visited[v]=true;list[r++]=v; //add something here.  Most importantly, you should add "v" into the list (which is a queue).
...
int main(){
	...
	for(int i=1;i<=n;i++){
		...
		for(int j=1;j<=d[i];j++){
		scanf("%d",&A[i][j]);
		B[i][j]=0;}// Add some code here. It inputs the data A[i][j] and assign B[i][j] = 0 for 1<=j<= d[i].
	}
	for(int i=1;i<=n;i++) 
