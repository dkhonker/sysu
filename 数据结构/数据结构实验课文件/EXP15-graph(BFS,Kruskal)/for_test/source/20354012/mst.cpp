...
int partition(int l, int r) {
	...
	for (int j = l; j < r; j++) 
		if (edge[ind[j]][2] < edge[ind[r]][2])
			swap(ind[j],ind[i++]);//add something here    Hint: Call swap() to exchange ind[j] and ind[r]
	swap(ind[i],ind[r]);//add something here       Hint: Call swap() exchange something.   (Recall what you learn from quicksort)
	return i;
}
...
void Qsort(int l, int r) {
	int k = partition(l, r);
	if(k!=l)Qsort(l,k-1);
	if(k!=r)Qsort(k+1,r);//add something here.   Hint : call Qsort recursively when left / right interval length larger than 1.
}
...
int kruskal() {
	...
	int sumCost = 0, cnt = 0;
	for (int i = 1; i <= m; i++) {
		int x = Find(edge[ind[i]][0]);
		int y = Find(edge[ind[i]][1]);
		if(x!=y){
		Union(x,y);cnt++;
		sumCost+=edge[ind[i]][2];}//add something here.   Hint: this is the major part of the Kruskal algorithm, but it is very very simple.
	}
