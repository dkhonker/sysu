...
int bfs(int u){
	...
	while (f < r){
		...
		for (int i=1; i<=d[u]; i++){
			...
			if(!visited[v]){
				list[r++]=v;
				visited[v]=true;//add something here.  Most importantly, you should add "v" into the list (which is a queue).
				B[u][v]=1;// Reminder: You have to change two entries of B when you select the edge (u,v).
...
void printAns(){
	...
	for(int i=1;i<=n;i++){
		for(int j=1;j<d[i];j++)
			printf("%d",B[i][j]);//add something here for print.
	} 
}
int main(){
	scanf("%d",&n);
