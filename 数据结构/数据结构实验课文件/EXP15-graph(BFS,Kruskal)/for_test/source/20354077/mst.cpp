...
int partition(int l, int r) {
	...
	for (int j = l; j < r; j++) 
		...
		{
				swap(edge[ind[j]][1],edge[ind[i]][1]);//add something here    Hint: Call swap() to exchange ind[j] and ind[r]
				swap(edge[ind[j]][0],edge[ind[i]][0]);
				swap(edge[ind[j]][2],edge[ind[i++]][2]);
			}
	            swap(edge[ind[r]][1],edge[ind[i]][1]);//add something here    Hint: Call swap() to exchange ind[j] and ind[r]
...
int kruskal() {
	...
	int sumCost = 0, cnt = 0;
	for (int i = 1; i <= m; i++) {
		int x=Find(edge[i][0]);//add something here.   Hint: this is the major part of the Kruskal algorithm, but it is very very simple.
		int y=Find(edge[i][1]);
		if(x!=y)
		{
			Union(x,y); cnt++;
