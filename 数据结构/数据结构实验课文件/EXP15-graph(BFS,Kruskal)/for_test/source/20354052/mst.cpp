...
int partition(int l, int r)
{
	...
	for (int j = l; j < r; j++) 
		if (edge[ind[j]][2] < edge[ind[r]][2])
			swap(ind[j],ind[r]);//add something here    Hint: Call swap() to exchange ind[j] and ind[r]
...
int kruskal() {
	...
	for (int i = 1; i <= m; i++)
	{
		...
		{
			sumCost+=edge[i][2];
			Union(Find(edge[i][0]),Find(edge[i][1]));
			cnt++;
		}
...
int main() {
	...
	for (int i = 1; i <= m; i++) ind[i]= edge[rank[i]][2];			   //After the sorting,  edge[ind[1]], edge[ind[2]],..., edge[ind[m]] are sorted by cost.
	int res = kruskal();
	printf("%d", res);
	return 0;
}
