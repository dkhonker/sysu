...
int partition(int l, int r) {
	...
	for (int j = l; j < r; j++) 
		if (edge[ind[j]][2] < edge[ind[r]][2])
			swap()//add something here    Hint: Call swap() to exchange ind[j] and ind[r]
...
void Qsort(int l, int r) {
	...
	if(k + 1 < r) Qsort(k + 1, l)//add something here.   Hint : call Qsort recursively when left / right interval length larger than 1.
}
