#include<stdio.h>
#include<memory.h>

const int maxn=401;
int edge[maxn][maxn];   //邻接矩阵
int color[maxn];        //0: uncolored.   -1 +1  : colored black or white.
int n;

bool dfs(int u,int c){
    color[u]=c; 
	int j=0;    //对u点进行染色操作 
    for(int v=1;v<=n;v++){//遍历与u点相连的点
        if(edge[u][v]==1){
            if(color[v]==c){
            	j++;
				break;//add some code here   (Hint: we can conlude that the graph is not bipartite now, as both u v are should be colored with c!)
            }
            //二分图的判定一般使用染色法，顾名思义，需要遍历图中的顶点，将某点u染色为1，
			//则所有与u连通的点都应该被染色为2，如果有点v与u连通，即uv是一条边，
			//但是v已经被染成和u一样的颜色了，则该图不是二分图。简而言之，尝试遍历图中每个连通块，
			//每轮染色要么失败，要么则将该连通块上所有的结点均染上颜色了。
			if(!color[v])
				color[v]=0;//add some code here.  (Hint: call dfs on v)
        }
	//add a return setence here.
    }
    if(j)
    return false;
	else
	return true;  
}

int main(){
    int k;
    scanf("%d",&k);
    for(int m=1;m<=k;m++){
    	scanf("%d",&n);
    	memset(edge,0,sizeof(edge));
        memset(color,0,sizeof(color));
        for(int i=1;i<=n;i++){
        	for(int j=1;j<=n;j++){
        		scanf("%d",&edge[i][j]);
			}
		} 
		//printf("sdygfuysfg");
		//Input the graph (scanf edge[i][j] ) by yourself in the following
		bool flag=true;
		for(int i=1;i<=n;i++){
			if(!color[i]){
			       //Add some code in the following. 
				if(!dfs(i,1)) {
				    flag=false;
				    break;
				}
					//Hint:  call dfs.  if it retruns false, print some information saying this is not a biparptite graph and REMEMBER TO break and set the flag variable to be false.
			} 
		}
		if(flag) //add some code in the folowing.
		printf("Yes\n");	//Hint: what shall be printed if flag=true?
	    else
	    printf("No\n");
	}
	return 0;
}
