...
bool dfs(int u,int c){	//深度优先搜索 depth first search 
    color[u]=c;     //对u点进行染色操作 
    for(int v=1;v<=n;v++)//遍历与u点相连的点
        if(edge[u][v]==1){
            if(color[v]==c) //假如v与u点相连 
            ...
            if(!color[v])
				return dfs(v,-c);//add some code here.  (Hint: call dfs on v)
        }
    return true;
...
int main(){
    ...
    for(int m=1;m<=k;m++){
		...
		bool flag=true;
		for(int i=1;i<=n;i++){
			if(!color[i]){ //Add some code in the following. 无色 
