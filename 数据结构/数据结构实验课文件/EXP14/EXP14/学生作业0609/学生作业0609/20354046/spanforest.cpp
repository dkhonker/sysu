...
int dfs(int u){
	...
	for(int i=1;i<=d[u];i++){
		if(!visited[A[u][i]]){
			B[u][i]=1;
			edges++;
			for(int k=1;k<=d[A[u][i]];k++)
				if(A[A[u][i]][k]==u)B[A[u][i]][k]=1;
			dfs(A[u][i]);
		}
	}//write this part by yourself
	return 1;//Hints, when u has an adjacent v which has not been visited, you should dfs(v) and choose the edge(u,v).
