...
int dfs(int v){
	visited[v]=1;//write this part by yourself
	for(int i=1;i<=d[v];i++)
	{
		int u=A[v][i];
       if(!visited[u])
...
void printAns(){
	printf("%d\n",edges);
	for(int i=1;i<=n;i++)//write this part. It prints the table B.
	{
	printf("%d ",d[i]);//remember to print d[i] first in the row corresponding to the i-th node.
	for(int j=1;j<=d[i];j++)
	printf("%d ",B[i][j]);
	printf("\n");
