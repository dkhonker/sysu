    ...
    for(int v=1;v<=n;v++)//遍历与u点相连的点
        if(edge[u][v]==1){
            if(color[v]==c) {
				...
				//add some code here   (Hint: we can conlude that the graph
				// is not bipartite now, as both u v are should be colored with c!)
            if(!color[v]){
            	if(!dfs(v,-c)){
            		return 0;
				}
    ...
    for(int m=1;m<=k;m++){
		...
		for(int i=1;i<=n;i++){
			if(!color[i]){ //Add some code in the following. 
					//Hint:  call dfs.  
					//if it retruns false, print some information saying this is 
					//not a biparptite graph and REMEMBER TO break and set the flag variable to be false.
				if(!dfs(i,1)){
					flag=0;
					printf("No\n");
					break;
