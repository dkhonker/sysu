...
int n, r[Maxn + 1], p[Maxn + 1];//任务i 只能在时刻 ri 后开始处理。
//任务i 需要 pi个时间单位才能完成。
...
int taskProcess() {
	...
	while (tasknum) {
		...
		//The following loop finds the task j 
		//with the earliest possible finishing time 
		//among those who can be processed now.
		for (int i = 1; i <= n; i++) {
